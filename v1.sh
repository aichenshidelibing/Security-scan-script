...
# v1.sh - Linux 基础安全加固 (v27.0 绝对不死版)
# 特性：移除 set -u 防闪退 | 退出强制暂停 | 全局单次自愈 | 28项全量

export LC_ALL=C

# =======================================================================
# [核心防闪退机制] 无论脚本因何种原因结束，都强制暂停，直到用户按回车
# =======================================================================
finish_trap() {
    echo -e "\n\033[33m[系统提示] 脚本运行结束或已停止。请按回车键关闭窗口...\033[0m"
    read -r
}
...
handle_lock() {
    ...
    if fuser "$lock" >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}锁未释放。请选: [1] 接着等 [2] 跳过 [3] 强制解锁(PID:$pid)${RESET}"
        read -p "选择: " c
        if [ "$c" == "3" ]; then
...
while true; do
    ...
    case "$ri" in
        ...
        r|R) [ -z "$SUM_IDS" ] && { MSG="请先勾选！"; continue; }
            ...
            # 强制不退出
            echo -ne "\n${YELLOW}【重要】所有流程执行完毕。按任意键返回主控台菜单...${RESET}"
            read -n 1 -s -r; exit 0 ;;
        *) for n in $ri; do 
